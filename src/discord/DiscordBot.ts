...
import { Client, Permissions, Guild, CommandInteraction, ApplicationCommandPermissionData, CreateRoleOptions, MessageEmbed, MessageActionRow, MessageButton, DiscordAPIError, Message, TextChannel, GuildMember, ButtonInteraction } from 'discord.js';
...
export class DiscordBot {
  ...
  async profile(interaction: GuildCommandInteraction) {
    await interaction.deferReply();

    const id = interaction.options.getString('player_id');

    const user = await UserSchema.findOne({ User: id });

    if (user) {
      const embed = new MessageEmbed().setTitle(`#${id}`).addField('Play Count', user.playCount.toString().addField('Passed', user.passedCount.toString(), true).addField('Unpassed', (user.playCount - user.passedCount).toString(), true).addField('Play Time', secondsToDhms(user.playTime)));
